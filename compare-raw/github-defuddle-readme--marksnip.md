...
Defuddle includes a command-line interface for parsing web pages directly from the terminal. You can run it with `npx` or [install it globally](https://github.com/kepano/defuddle#cli-installation). The CLI accepts a file path, a URL, or HTML piped over stdin.
