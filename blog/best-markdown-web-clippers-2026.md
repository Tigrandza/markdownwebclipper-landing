...
- **Falls short:** no GitHub Gist destination, no batch-clip-many-tabs. Extraction runs on
  Defuddle, which Kepano wrote for this clipper — the same engine ours uses, so on
  article-shaped pages both tools start from the same extracted content.
- **Pricing:** free.
...
- **Best at:** high-fidelity extraction of the hard cases (rowspan tables, KaTeX math, fenced
  code with language hints, footnotes) plus a real destination set. The extractor is the same
  Defuddle the official Obsidian clipper uses; the difference is the repair work wrapped around
  it, and conversion by Turndown with the Joplin GFM plugin. A pre-extraction pass rescues
  MediaWiki fraction markup; post-extraction passes fix the spaces injected around `<sub>` and
  `<sup>` and rejoin formulas stacked across a line break; a math rule unwraps MediaWiki's
  `{\displaystyle …}` and `\ce{…}` so mhchem and KaTeX can render the result; and Turndown
  rules turn Defuddle's footnote anchors into real `[^1]` footnotes instead of dead links.
  Three of those are defects in Defuddle itself rather than in any clipper built on it. Each
  reproduces from the library with a minimal fixture, and each is filed upstream with that
  reproduction: kepano/defuddle#366, #367 and #369. The local work is a stopgap. The fraction
  and sub/sup passes are written to be deleted outright once #366 and #367 land, and #369 can
  only be fixed upstream, because the markup it concerns is gone before the content reaches
  us. The current state of all three is public at those links. On ChatGPT, Claude and Gemini a bounded
  scroll-and-expand pass runs before extraction so a virtualized conversation is fully in the
  DOM first. Five destinations: local folder (File System Access API — direct vault write, no
  sync server), clipboard, .md download, Obsidian via `obsidian://` URI, and (Pro) GitHub Gist
  with revision history via `PATCH /gists/{id}`.
...
|---|---|---|---|---|
| Obsidian Web Clipper | Defuddle — the same extractor we use | Obsidian vault (native), clipboard, download | Yes (MIT) | Free |
...
| Markdown Web Clipper | Defuddle + GFM, plus repair passes for fractions, sub/sup, math and footnotes | Local folder (FSA), clipboard, download, Obsidian, Gist (Pro) | Yes (MIT, free edition) | Free + lifetime $9 / $19 Pro |
...
_**Correction, 2026-08-22:** an earlier version of this post said the official Obsidian Web
Clipper extracts with Mozilla Readability. That was wrong. It uses Defuddle — the same
extractor this clipper uses, written by Kepano for the official clipper. The Obsidian Web
Clipper and comparison-table entries have been rewritten accordingly._
